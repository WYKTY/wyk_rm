        ...
        while (1)
        {
            ...
            for (const auto& contour : contours)
            {
                ...
                if(SideLength1>=SideLength2)
                {
                    ...
                    if(ell.angle>65&&ell.angle<125) continue;
                    if(LenRatio>1.2&&LenRatio<10)
                    {
                        ellipse(img, ell, Scalar(255, 255, 255));
#if VISUALDATA
                        float S1=Xdiff1/Ydiff1;
                        string ang=to_string(S1);
                        putText(img, ang, vertex[2], FONT_HERSHEY_COMPLEX, 0.5, Scalar(0, 255, 255), 1, 8);
#endif
...
//                    if (abs(FTop.y - STop.y) > (len / 2.3))//  height//1.7
//                    {
...
//                        continue;
//                        if(abs(FirdeltaX*SecdeltaX+FirdeltaY*SecdeltaY)/(len*dis)<0.5)
//                        {
//                            line(img, FBot, SBot, Scalar(255, 255, 255));
//                            putText(img, fin1, Mid1, FONT_HERSHEY_COMPLEX, 0.5, Scalar(0, 255, 255), 1, 8);
//                            continue;
...
//                    }
                    ...
                    if (abs(FirInvSlope - SecInvSlope) < 0.2)// 原初1.5   -
                    {

                        double ju;
                        double v_f_x=FTop.x-FBot.x;
                        double v_f_y=FTop.y-FBot.y;
                        double v_s_x=STop.x-FTop.x;
                        double v_s_y=STop.y-FTop.y;
                        ju=v_f_x*v_s_x+v_f_y*v_s_y;
                        ju/=len;
                        ju/=dis;
                        ju=abs(ju);
                        //cout<<ju<<"\n";
                        if (abs(FTop.y - STop.y) > (len / 2.3))//  height//1.7
                        {
                            ...
                            string mas=to_string(ju);
                            putText(img, mas, FBot, FONT_HERSHEY_COMPLEX, 0.5, Scalar(0, 255, 255), 1, 8);
                            if(ju>0.15)
                            {
                                continue;
                            }
...
//                    }
            ...
            if (if_play)
            {
                imshow("mark", img);
#ifndef V2
                int curr_pos = (int)video.get(CAP_PROP_POS_FRAMES);
                setTrackbarPos("process", "mark", curr_pos);
#endif
                waitKey(1);
                imshow("Aim", Aim);
                waitKey(1);
